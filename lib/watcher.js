...
exports.isConflictFile = function( filename ) {
...
function getPathWatcherThroughRequire(modulePath) {
...
function initPathWatcher() {
	try {
		...
		pathwatcher = getPathWatcherThroughRequire( "pathwatcher" );
		if ( pathwatcher ) {
		...
		logger.finest( e );
		logger.finer( "Unable to find pre-compiled pathwatcher. Disabling watching capability." );
	}
	return false;
}
/*
		...
		this.addHandler = function( pathAndFilename ) {
			...
			if ( fs.existsSync( pathAndFilename ) ) {
				if ( fs.lstatSync( pathAndFilename ).isDirectory() ) {
					var dirhandler = new DirHandler( filename, that );
					dirhandler.walk();
					that.handlerList.push( dirhandler );
				} else {
...
*/
	...
	if ( disallowedFolders.length > 0 ) {
		...
		for ( var j = 0; j < disallowedFolders.length; j++ ) {
			logger.info( disallowedFolders[j] );
		}
		logger.info( " " );
