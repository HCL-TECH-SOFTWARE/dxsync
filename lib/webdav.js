...
function isCollection( webDavObject ) {
	...
	propstat = Array.isArray( propstat ) ? propstat : [propstat];
		...
		if ( method == "PUT" ) {
			...
			if ( !Buffer.isBuffer( options.body ) ) {
				if ( Array.isArray( options.body ) ) {
					...
					length = Buffer.from( options.body ).length;
				}
			} else {
			...
			xml2js.parseString( xmlBody, {
				...
				if ( err ) {
					...
					if ( singlefile ) {
						...
						n = splitWebDavPath( decodeURIComponent( response["href"] ) );
						...
						if ( Array.isArray( responseArray ) ) {
							...
							for ( var i = 1; i < responseArray.length; i++ ) {
								...
								n = splitWebDavPath( decodeURIComponent( responseArray[i]["href"] ) );
