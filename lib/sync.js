...
var webdav = require( "./webdav.js" ),
	...
	nls = utils.nls,
	notifyIcon = utils.NOTIFI_ICON.notifyIcon,
	conflictIcon = utils.NOTIFI_ICON.conflictIcon,
	errorIcon = utils.NOTIFI_ICON.errorIcon;
...
function deleteConflictFile( filename ) {
	var conflictFile = filename + SUFFIX_CONFLICT;
	return fs.removeSync( conflictFile );
}
...
var SyncProcess = function( _config, _eventEmitter ) {
	...
	var backgroundSyncHandler = new BackgroundSyncHandler();
	...
	this.sync = function( mode, autowatch, bBackgroundSync ) {
		...
		var recurseLocale = function( filename ) {
			...
			function recurseLocaleDir( path, baseFilename ) {
			...
			function recurseLocaleFile( path ) {
			...
			for ( var i = 0; i < list.length; i++ ) {
				...
				if ( fs.existsSync( path ) && fs.lstatSync( path ).isDirectory() ) {
					...
					// need to remember the entry, otherwise it is overwritten through the loop
					recurseLocaleDir( path, baseFilename );
				} else {
					if ( remoteFileMap[baseFilename] ) {
	...
	*/
					...
					if ( localHash == null || ( localSize === 0 && hasRecognizedConflict ) ) {
						...
						if ( mode == MODE_PUSH ) {
							...
							if ( !hasRecognizedConflict ) {
								conflictFile = filename + SUFFIX_CONFLICT;
	...
	function watch() {
		...
		var handleAdd = function( path, isDir ) {
			...
			if ( !isDir ) {
				var errorHandlerDir = function( err ) {
					deferred.reject( {
						filename: path,
						error: err
					} );
				...
				var errorHandlerFile = function( err ) {
					deferred.reject( {
		...
		var handleDelete = function( path, isDir ) {
			...
			if ( !isDir ) {
				...
				var  errorHanderFile = function( err ) {
					deferred.reject( {
						filename: baseFilename,
						error: err
					} );
		...
		var handleRename = function( oldpath, newpath, isDir ) {
			...
			if ( hasConflict( oldpath ) ) {
				if ( takeResolveAction( oldpath ) ) {
					var errorHandlerConflict = function( err ) {
						deferred.reject( {
				...
				if ( utils.isHidden( newpath, config ) ) {
					...
					var errorHandlerRename = function( err ) {
...
exports.startTwoWaySync = function( config, autowatch, bBackgroundSync, eventEmitter ) {
	lastSyncProcess = new SyncProcess( config, eventEmitter );
	return lastSyncProcess.sync( MODE_TWOWAY, autowatch, bBackgroundSync );
}

exports.startPullSync = function( config, autowatch, bBackgroundSync, eventEmitter ) {
	lastSyncProcess = new SyncProcess( config, eventEmitter );
	return lastSyncProcess.sync( MODE_PULL, autowatch, bBackgroundSync );
}

exports.startPushSync = function( config, autowatch, bBackgroundSync, eventEmitter ) {
	lastSyncProcess = new SyncProcess( config, eventEmitter );
	return lastSyncProcess.sync( MODE_PUSH, autowatch, bBackgroundSync );
}

// purely for testing, get event emitter
exports.getEventEmitter = function() {
	console.log( "TESTING PURPOSE ONLY - get event emitter..." );
	return lastSyncProcess ? lastSyncProcess.getEventEmitter() : null;
}
