...
describe( "sync-basics", function() {
	...
	describe( "error-cases", function() {
		...
		it( "Should display uncaught exception", function( done ) {
			...
			process.exit = function( code ) {
				process.exit = oldExit;
				hookErr.captured().should.containEql( "Uncaught Exception" );
				hookErr.captured().should.containEql( "test error" );
				done();
			}
			fn( "test error" );
		} )
		...
		it( "Should fail to read config", function( done ) {
			syncPoint = helper.createSyncPoint( function() {
				...
				helper.cleanupSyncPoint( function() {
					hookErr.captured().should.containEql( "Unable to load dxsync settings in this directory." );
					done();
				} );
		...
		it( "Simply execute notification without verifying it", function( done ) {
			syncUtils.notify( {
				title: "testTitle",
				message: "testMessage",
				icon: syncUtils.NOTIFI_ICON.notifyIcon
			} );
			done();
		} )
	...
	describe( "utils", function() {
		...
		it( "Should create multiple level of directories", function( done ) {
			...
			syncPoint = helper.createSyncPointWithContent( "local-noconfig", function() {
				...
				syncUtils.writeConfig( syncPoint, config, function() {
					...
					config.password.should.be.exactly( "password" );
		...
		it( "Should format the string correctly", function( ) {
			...
			result = syncUtils.format( "Inspect Object %t", { a: 2 } );
			console.log( result );
			result.should.be.exactly( "Inspect Object { a: 2 }" );

			result = syncUtils.format( "Unknown %x", "Hello", { a: 2 } );
			console.log( result );
			result.should.be.exactly( "Unknown %x Hello { a: 2 }" );

		} );

		it( "Should load english nls file in all cases", function( ) {

			var loadNlsFile = nlsUtils.loadNlsFile,
