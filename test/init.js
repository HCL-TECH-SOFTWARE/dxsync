	...
	describe( "init-with-server-nothemes", function() {
		...
		it( "Should say no themes if none are available", function( done ) {
			...
			setTimeout( function() {
				...
				setTimeout( function() {
					hook.captured().should.containEql( "Connecting to the remote server..." );
					hookErr.captured().should.containEql( "No themes found on the target server." );
					done();
				}, 500 );
			}, 500 );
		...
		it( "Should fail due to invalid sync point", function( done ) {
			...
			setTimeout( function() {
				...
				setTimeout( function() {
					...
					setTimeout( function() {
						hookErr.captured().should.containEql( "Aborting initialization." );
						hookErr.captured().should.containEql( path.normalize( syncPoint + "/notExists" ) );
